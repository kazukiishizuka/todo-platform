...
@router.post("/events")
async def slack_events(request: Request, x_slack_request_timestamp: str = Header(default=""), x_slack_signature: str = Header(default=""), repository=Depends(get_repository), slack_client=Depends(get_slack_client)):
...
async def slack_interactions(request: Request, repository=Depends(get_repository)):
